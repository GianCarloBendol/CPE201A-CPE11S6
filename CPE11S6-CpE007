...
int main() {
    ...
    if (category == 1) items = gpu, size = 5;
    else if (category == 2) items = cpu, size = 4;
    else if (category == 3) items = psu, size = 3;
    else if (category == 4) items = ram, size = 4;
    else if (category == 5) items = input, size = 6;
    else { cout << "Invalid category.\n"; break; }

    for (int i = 0; i < size; i++)
        cout << i + 1 << ". " << items[i].productModel
             << " (Stock: " << items[i].stockCount
             << ", Price: " << items[i].price << ")\n";
  
    int choice;
    cout << "Enter product number: ";
    cin >> choice;
    if (choice < 1 || choice > size) { cout << "Invalid choice.\n"; break; }

    Product &p = items[choice - 1];
    ...
    if (qty > p.stockCount) { cout << "Not enough stock.\n"; break; }
 
    double total = p.price * qty;
    double profit = (p.price - (p.cost == 0 ? p.price * 0.7 : p.cost)) * qty;

    p.stockCount -= qty;
    totalProfit += profit;

    cout << "\nSale completed!\n";
    cout << "Product: " << p.productModel << endl;
    cout << "Sold: " << qty << endl;
    cout << "Total sale: ₱" << total << endl;
    cout << "Profit earned: ₱" << profit << endl;
    cout << "Remaining stock: " << p.stockCount << endl;
    cout << "Overall profit: ₱" << totalProfit << endl;
    ...
    case 3: {
        ...
        cout << "2. Processors\n";
        cout << "3. Power Supply\n";
        cout << "4. Memory Modules\n";
        cout << "5. Input Devices\n";
        ...
        cout << "\nAvailable Products:\n";
        for (int i = 0; i < size; i++) {
            if (products[i].cost == 0) {
                products[i].cost = products[i].price * 0.7;
            }
    
            cout << i + 1 << ". " << products[i].productModel
                 << " | Stock: " << products[i].stockCount
                 << " | Price: $" << fixed << setprecision(2) << products[i].price
                 << " | Cost: $" << fixed << setprecision(2) << products[i].cost
                 << endl;
        }
    
        int productChoice;
        cout << "\nSelect a product number to restock: ";
        cin >> productChoice;
        ...
        while (true) {
            ...
            if (quantity > supplierLimit) {
                cout << "[ERROR] Supplier stock is insufficient.\n";
                continue;
            }
            ...
            if (option == 1) {
                ...
                double markupPercent = (rand() % 16) / 100.0;
                selectedProduct.price = selectedProduct.cost * (1 + markupPercent);
                ...
                cout << "New selling price : $"
                     << fixed << setprecision(2) << selectedProduct.price << "\n";
                break;
            } else if (option == 2) {
                continue;
            } else if (option == 3) {
